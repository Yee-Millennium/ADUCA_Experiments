...
def aduca_scale(problem: GMVIProblem, exit_criterion: ExitCriterion, parameters, u_0=None):
    ...
    normalizers = np.copy(param_L)
    normalizers_recip = np.copy(1/param_L)
    ...
    while True:
        ...
        for index,block in enumerate(blocks):
            if index != 0:
                problem.operator_func.func_map_block_update(F_store, u_1[block], Q, block)
            F_tilde_1[block] = F_store[block]
            Q += np.sum(u_1[block] - u[block])
    ...
    while not exit_flag:
        ...
        for index, block in enumerate(blocks, start=0):
            if index !=0:
                problem.operator_func.func_map_block_update(F_store, u[block], Q, block)
            F_tilde_[block] = F_tilde[block]
            F_tilde[block] = F_store[block]
