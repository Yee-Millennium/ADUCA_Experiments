...
def gr(problem: CompositeFunc, exit_criterion: ExitCriterion, parameters, x_0=None):
    # Init of Golden-Ratio
    beta = parameters["beta"]
    rho = beta + beta**2
    if x_0 is None:
        x_0 = np.zeros(problem.d)
    x_1 = np.ones(problem.d)
    ...
    def gr_stepsize(x, x_minus_1, a_step):
        ...
        delta_grad = problem.grad(x) - problem.grad(x_minus_1)
    ...
    while not exit_flag:
        ...
        x = problem.reg_func.prox_opr((v - step*problem.grad(x_minus_1)), step)
